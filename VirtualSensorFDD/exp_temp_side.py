...
data = pd.read_csv(r'C:\Users\user\OneDrive - Chonnam National University\학석사 연계과정\코드\FDD python\samsung\Experiment\Data\2022-{}-{}\{}{} temp2.csv'.format(month, day, month, day), engine='python').fillna(method='bfill').fillna(method='ffill')
...
y1 = y1.point1.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y2.index = pd.to_datetime(y2['updated_time'])
y2.drop('updated_time', inplace=True, axis=1)
y2 = y2.point2.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y3.index = pd.to_datetime(y3['updated_time'])
y3.drop('updated_time', inplace=True, axis=1)
y3 = y3.point3.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y4.index = pd.to_datetime(y4['updated_time'])
y4.drop('updated_time', inplace=True, axis=1)
y4 = y4.point4.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y5.index = pd.to_datetime(y5['updated_time'])
y5.drop('updated_time', inplace=True, axis=1)
y5 = y5.point5.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y6.index = pd.to_datetime(y6['updated_time'])
y6.drop('updated_time', inplace=True, axis=1)
y6 = y6.point6.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y7.index = pd.to_datetime(y7['updated_time'])
y7.drop('updated_time', inplace=True, axis=1)
y7 = y7.point7.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y8.index = pd.to_datetime(y8['updated_time'])
y8.drop('updated_time', inplace=True, axis=1)
y8 = y8.point8.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
y9.index = pd.to_datetime(y9['updated_time'])
y9.drop('updated_time', inplace=True, axis=1)
y9 = y9.point9.resample('1T').mean().apply(lambda x: None if x < -10 or x > 15 else x).fillna(method='bfill').fillna(method='ffill')
...
overall = pd.concat([y1, y2, y3, y4, y5, y6, y7, y8, y9 ], axis=1).to_csv('./temp_resample_side.csv')

power.index = pd.to_datetime(power['updated_time'])
power = power.loc[y1.index[0]:y1.index[len(y1) - 1], 'value']

# ,statistics.mean(y8),statistics.mean(y10), statistics.mean(y11), statistics.mean(y12),
#                        statistics.mean(y13), statistics.mean(y14), statistics.mean(y15), statistics.mean(y16),
#                        statistics.mean(y17), statistics.mean(y18), statistics.mean(y19), statistics.mean(y20),
#                        statistics.mean(y21), statistics.mean(y22), statistics.mean(y23), statistics.mean(y24)
...
fig, ax1 = plt.subplots(4, 5, figsize=(16, 12))
