...
for loop in range(6):
    ...
    data = pd.read_csv(r'C:\Users\user\OneDrive - Chonnam National University\학석사 연계과정\코드\FDD python\samsung\Experiment\Data\2021-08-13\0813 flow.csv', engine='python')
    data_point = pd.read_csv(r'C:\Users\user\OneDrive - Chonnam National University\학석사 연계과정\코드\FDD python\samsung\Experiment\Data\2021-08-13\point_time_side_3067.csv')
    power = pd.read_csv(r'C:\Users\user\OneDrive - Chonnam National University\학석사 연계과정\코드\FDD python\samsung\Experiment\Data\2021-08-13\outdoor_3066.csv')
    ...
    for i in range(3):
        for j in range(1):
            if rolling:
                ...
                # ax1[i,j].plot(range(len(y[high_index[k]+30:high_finish_index[k]])),y[high_index[k]+30:high_finish_index[k]], color='r',linewidth=1.5, linestyle='-')
            else:
                ax1[i].plot(range(len(y[normal_index[k]:normal_finish_index[k]])),y[normal_index[k]:normal_finish_index[k]], color='b', linewidth=1.5, linestyle='-')
                # ax1.plot(range(len(y[low_index[k]:low_finish_index[k]])),y[low_index[k]:low_finish_index[k]], color='g',linewidth=1.5, linestyle='-')
                # ax1[i, j].plot(range(len(y[high_index[k]:high_finish_index[k]])),y[high_index[k]:high_finish_index[k]], color='r', linewidth=1.5, linestyle='-')
            ax2 = ax1[i].twinx()
            ...
            ax1[i].set_yticks([-2, -1.5, -1, -0.5, 0, 0.5, 1.0, 1.5, 2.0])
            ...
            if POWER:
                ax2.set_yticks([0,3000,6000,9000,12000,15000,18000,21000,24000,27000,30000,33000,36000])
            elif not POWER:
            ...
            if i == 0:
                if POWER:
                    ...
                    if legend:
                        ax2.legend([
                                    'Experiment 2 (Power) - {}~{}'.format(high.values[0],high_finish.values[-1])],
                                   bbox_to_anchor=(1, -1))
                elif not POWER:
                    ax2.set_ylabel('Fan step', fontsize=14)
                    if legend:
                        ax2.legend([
                                    'Experiment 2 (Fan step) - {}~{}'.format(high.values[0],high_finish.values[-1])],
                                   bbox_to_anchor=(1, -1))
            k += 1
    ...
    if legend:
        ax1[0].legend([
                         'Experiment 2 (Air velocity) - {}~{}'.format(high.values[0],high_finish.values[-1])],bbox_to_anchor=(0, -1))

    ax1[1].set_ylabel('Air Velocity [m/s]',fontsize=14)
    ax1[0].set_title('Air velocity (side)', fontsize=16)
