...
for loop in range(4):
    ...
    data_point_normal = pd.read_csv(r'C:\Users\user\OneDrive - Chonnam National University\학석사 연계과정\코드\FDD python\samsung\Experiment\Data\2021-07-28\point_time_upper.csv')
    ...
    data_point_fault = pd.read_csv(r'C:\Users\user\OneDrive - Chonnam National University\학석사 연계과정\코드\FDD python\samsung\Experiment\Data\2021-08-05\point_time_upper.csv')
    ...
    for i in range(len(power_normal)):
        for j in range(len(data_point_normal)):
            if power_normal['Time'][i] == normal_finish_normal[j][0:5]+':00':
                ...
                power_finish_high_normal.append(i-1)
    ...
    for j in range(1):
        ...
        y_normal_normal = [y_n[normal_index_normal[k]:normal_finish_index_normal[k]].values.tolist()[l] for l in
                           range(len(y_n[normal_index_normal[k]:normal_finish_index_normal[k]].values.tolist())) for n
                           in range(3)]
        x_normal_low = range(len(y_n[low_index_normal[k] + 30:low_finish_index_normal[k]]) * 3)
        y_normal_low = [y_n[low_index_normal[k] + 30:low_finish_index_normal[k]].values.tolist()[l] for l in
                        range(len(y_n[low_index_normal[k] + 30:low_finish_index_normal[k]].values.tolist())) for n in
                        range(3)]
        x_normal_high = range(len(y_n[high_index_normal[k] + 30:high_finish_index_normal[k]]) * 3)
        y_normal_high = [y_n[high_index_normal[k] + 30:high_finish_index_normal[k]].values.tolist()[l] for l in
                         range(len(y_n[high_index_normal[k] + 30:high_finish_index_normal[k]].values.tolist())) for n in
                         range(3)]
        ...
        ax1.plot(x_normal_normal, y_normal_normal, color='b', linewidth=1.5, linestyle='-')
        ax1.plot(x_normal_low, y_normal_low, color='g', linewidth=1.5, linestyle='-')
        ax1.plot(x_normal_high, y_normal_high, color='r', linewidth=1.5, linestyle='-')
        ax1.plot(x_fault_normal, y_fault_normal, color='b', linewidth=1.5, linestyle='--')
        # ax1.plot(x_fault_low, y_fault_low, color='g', linewidth=1.5, linestyle='--')
        # ax1.plot(x_fault_high, y_fault_high, color='r', linewidth=1.5, linestyle='--')
        ...
        py_normal_normal = [y2_n[power_index_normal[k]:power_finish_normal[k]].values.tolist()[l] for l in
                            range(len(y2_n[power_index_normal[k]:power_finish_normal[k]].values.tolist())) for m in
                            range(90)]
        px_normal_low = range(len(y2_n[power_index_low_normal[k]:power_finish_low_normal[k]]) * 90)
        py_normal_low = [y2_n[power_index_low_normal[k]:power_finish_low_normal[k]].values.tolist()[l] for l in
                         range(len(y2_n[power_index_low_normal[k]:power_finish_low_normal[k]].values.tolist())) for m in
                         range(90)]
        px_normal_high = range(len(y2_n[power_index_high_normal[k]:power_finish_high_normal[k]]) * 90)
        py_normal_high = [y2_n[power_index_high_normal[k]:power_finish_high_normal[k]].values.tolist()[l] for l in
                          range(len(y2_n[power_index_high_normal[k]:power_finish_high_normal[k]].values.tolist())) for m
                          in range(90)]
        ...
        ax1.set_xticks([0, 150, 300, 450, 600, 750, 899])
        ax1.set_xticklabels([x[n].strftime('%H:%M:%S') for n in range(len(x)) if n % 50 == 0])

        if POWER:
            ax2.set_ylabel('Power [kW]', fontsize=14)
            if legend:
                ax2.legend(['No fault conditions 1 (Power) - {}~{}'.format(normal_normal.values[0],
                                                                           normal_finish_normal.values[-1]),
                            'No fault conditions 2 (Power) - {}~{}'.format(low_normal.values[0],
                                                                           low_finish_normal.values[-1]),
                            'No fault conditions 3 (Power) - {}~{}'.format(high_normal.values[0],
                                                                           high_finish_normal.values[-1]),
                            'Fault conditions  1 (Power) - {}~{}'.format(normal_fault.values[0],
                                                                         normal_finish_fault.values[-1]),
                            'Fault conditions 2 (Power) - {}~{}'.format(low_fault.values[0],
                                                                        low_finish_fault.values[-1]),
                            'Fault conditions 3 (Power) - {}~{}'.format(high_fault.values[0],
                                                                        high_finish_fault.values[-1])],
                           bbox_to_anchor=(1, -1))
        elif not POWER:
            ax2.set_ylabel('Fan step', fontsize=14)
            if legend:
                ax2.legend(['No fault conditions 1 (Fan step) - {}~{}'.format(normal_normal.values[0],
                                                                              normal_finish_normal.values[-1]),
                            'No fault conditions 2 (Fan step) - {}~{}'.format(low_normal.values[0],
                                                                              low_finish_normal.values[-1]),
                            'No fault conditions 3 (Fan step) - {}~{}'.format(high_normal.values[0],
                                                                              high_finish_normal.values[-1]),
                            'Fault conditions 1 (Fan step) - {}~{}'.format(normal_fault.values[0],
                                                                           normal_finish_fault.values[-1]),
                            'Fault conditions 2 (Fan step) - {}~{}'.format(low_fault.values[0],
                                                                           low_finish_fault.values[-1]),
                            'Fault conditions 3 (Fan step) - {}~{}'.format(high_fault.values[0],
                                                                           high_finish_fault.values[-1])],
                           bbox_to_anchor=(1, -1))
        k += 1
    ...
    #Air velocity
    if legend:
        ax1.legend(['No fault conditions 1 (Air volume) - {}~{}'.format(normal_normal.values[0],normal_finish_normal.values[-1]),
                         'No fault conditions 1 (Air volume) - {}~{}'.format(low_normal.values[0],low_finish_normal.values[-1]),
                         'No fault conditions 2 (Air volume) - {}~{}'.format(high_normal.values[0],high_finish_normal.values[-1]),
                         'Fault conditions 1 (Air volume) - {}~{}'.format(normal_fault.values[0],normal_finish_fault.values[-1]),
                         'Fault conditions 2 (Air volume) - {}~{}'.format(low_fault.values[0],low_finish_fault.values[-1]),
                         'Fault conditions 3 (Air volume) - {}~{}'.format(high_fault.values[0],high_finish_fault.values[-1])],
                        bbox_to_anchor=(0, -1))
    ...
    if POWER:
        if legend:
            plt.savefig(directory + '\MassFlow_upper_legend.png', dpi=300)
        else:
            plt.savefig(directory + '\MassFlow_upper_ma.png', dpi=300)

    elif not POWER:
        if legend:
            plt.savefig(directory + '\MassFlow_upper_legend_fanstep.png', dpi=300)
        else:
